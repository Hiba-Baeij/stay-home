...
export default function DeliveryOrder() {
    ...
    const { isLoading } = useQuery(['order'], OrderApi.getAllDeliveryOrder, {
        onSuccess: (data: { response: Order[]; }) => {
            dispatch(orderActions.setDeliveryOrder(data.response))
        },
    })
...

                {
                    isLoading ? <TableSkeleton headers={['', 'اسم الزبون', ' الوجهة', 'المصدر', 'حالة الطلب', 'اسم المتجر', 'تفاصيل']} />
                        ...
                        : <Table sx={{ minWidth: 650 }} aria-label="simple table">
                            ...
                            <TableBody>
                                {deliveryOrders ? paginate(deliveryOrders).map((row: Order, index: number) => {
                                    return (
                                        <TableRow
                                            hover
                                            role="checkbox"
